...
#if IS_ENABLED(CONFIG_ZMK_BEHAVIOR_ONESHOT_MODIFIERS)
#include <zmk/behaviors/sticky_mods.h>
#endif
...
static struct modifiers_status_state modifiers_status_get_state(const zmk_event_t *eh) {
    zmk_mod_flags_t aggregated_modifiers = zmk_hid_get_keyboard_report()->body.modifiers;
    aggregated_modifiers |= zmk_behaviors_sticky_mods_get_current_combo();

    return (struct modifiers_status_state){.modifiers = aggregated_modifiers};
}
